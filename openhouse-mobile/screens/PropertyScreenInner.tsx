...
export type StripeSheetActions = {
  initPaymentSheet: (options: {
    ...
    defaultBillingDetails?: { name: string };
  }) => Promise<{ error?: { message?: string } | null }>;
  presentPaymentSheet: () => Promise<{ error?: { message?: string } | null }>;
};

type InnerProps = Props & { stripe: StripeSheetActions | null };
...
export default function PropertyScreenInner({ route, navigation, stripe }: InnerProps) {
  ...
  const handleBuyReport = async () => {
    ...
    if (!trimmed) {
      Alert.alert(t("prop_email_missing_title"), t("prop_email_missing_msg"));
      return;
    }
    if (!stripe) {
      Alert.alert(t("prop_pay_error_title"), t("prop_web_pay_only"));
      return;
    }
    setError(null);
    setPayLoading(true);
    try {
      ...
      const { error: payErr } = await stripe.presentPaymentSheet();
      if (payErr) {
        Alert.alert(t("prop_pay_error_title"), payErr.message || t("prop_pay_cancelled"));
        return;
      }
      Alert.alert(t("prop_pay_success_title"), t("prop_pay_success_msg"));
      (navigation as { navigate: (name: string) => void }).navigate("Success");
    } catch (err) {
      setError(err instanceof Error ? err.message : t("prop_pay_cancelled"));
    } finally {
      setPayLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        <Text style={styles.title}>{t("prop_title")}</Text>
        <View style={styles.cardCatastro}>
          <Text style={styles.cardTitle}>{t("prop_catastro_card")}</Text>
          <Text style={styles.label}>{t("prop_address_label")}</Text>
          <Text style={styles.value}>{property.address || "—"}</Text>
          <Text style={styles.label}>{t("prop_year_label")}</Text>
          <Text style={styles.value}>{displayYear}</Text>
          <Text style={styles.label}>{t("prop_score_label")}</Text>
          {scorOportunitate > 0 ? (
            <>
              <Animated.Text
                style={[
              ...
              {showPotentialText && (
                <Text style={styles.potentialText}>{t("prop_potential_text")}</Text>
              )}
            </>
          ) : (
            <Text style={styles.value}>—</Text>
          )}
          <Text style={styles.label}>{t("prop_ref_label")}</Text>
          <Text style={[styles.value, styles.valueCode]}>{property.ref_catastral || "—"}</Text>
        </View>
        <Text style={styles.label}>{t("prop_email_label")}</Text>
        <TextInput
          style={styles.input}
          value={email}
          onChangeText={(t) => {
        ...
        {error && (
          <View style={styles.errorBox}>
            ...
            <TouchableOpacity onPress={handleBuyReport} disabled={payLoading}>
              <Text style={styles.retryText}>{t("prop_retry")}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <TouchableOpacity style={styles.button} onPress={handleBuyReport} disabled={payLoading}>
          {payLoading ? (
            ...
            <Text style={styles.buttonText}>{t("buy_report_button")}</Text>
          )}
        </TouchableOpacity>
        <Text style={styles.guaranteeText}>{t("guarantee_official")}</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  valueProp: { fontSize: 12, color: colors.textMuted, fontStyle: "italic", marginTop: spacing.md, marginBottom: spacing.sm },
  button: {
