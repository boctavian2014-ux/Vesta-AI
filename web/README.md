...
| `VESTA_STRICT_STARTUP` | (Optional) In **production** only: set to `1`, `true`, or `yes` to **exit on boot** if `VEST_PYTHON_API_URL` is empty (fail-fast so the service is not “green” but unusable). See also [`docs/PRODUCTION_CHECKLIST.md`](../docs/PRODUCTION_CHECKLIST.md). |
